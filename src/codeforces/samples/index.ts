...
const getMakefile = (id: string, problem: string) =>
  MAKEFILE +
  `
submit:
\tcf-crawler submit -p=cf -i=${id} -P=${problem} sol.cpp
`
...
const processProblem = async (
  id: string,
  problem: { label: string; url: string }
) => {
  ...
  pendingPromises.push(
    fs.writeFile(`${problemFolder}/Makefile`, getMakefile(id, problem.label))
  )
...
export const handleSamples = async (id: string | undefined) => {
  if (!id) {
  ...
  await Promise.all(problems.map((problem) => processProblem(id, problem)))
}
